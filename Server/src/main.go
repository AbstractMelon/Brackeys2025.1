...
func setupLogging(cfg *config.ServerConfig) *logger.Logger {
	l, err := logger.NewLogger(cfg.LogFile, cfg.EnableDebug)
	if err != nil {
...
func main() {
	...
	l.Info("Server listening on %s", listener.Addr().String())
	...
	for {
		...
		if err != nil {
			l.Warning("Accept error: %v", err)
			continue
		}
		clientHandler := handlers.NewClientHandler(conn, state, cfg, l)
